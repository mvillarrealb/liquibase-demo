...
Wheter you are starting on a new project or mantaining an existing codebase, mantaining database changes as controlled and reproducible as possible is a **must**, there are a varieity of tools for achieving this goal but today we going to focus on [liquibase](https://www.liquibase.org/), an opensource(and paid) database versioning tool.
...
Liquibase enable a variety of formats for your changelog(sql, xml, properties and yaml files) in this case and since we are using spring and micronaut, we are going to create yaml files as our changelog.
...
And of course we need a initial database version.
...
To download my setup [micronaut launch project](https://micronaut.io/launch?javaVersion=JDK_11&lang=JAVA&build=GRADLE&test=JUNIT&name=book-demo-micronaut&package=org.mvillabe.books&type=DEFAULT&features=liquibase&features=postgres&features=data-jpa&features=testcontainers&version=2.5.4&activity=preview&showing=README.md)


# Creating Database Migrations for a existing Database

If you are on a project with an existing database with no versioning at all, fear not, we can create a changelog with liquibase, for this case we are going to need the [liquibase installed](https://www.liquibase.org/download). 

Once installed we need to execute the following steps:
...
* When using liquibase, each executed version is **inmutable**, each version creates a hash of the file, if you modify it it will change the hash and causing a corrupted changelog.
...
* If you feel uncomfortable adding liquibase to your project runtime, you can use gradle or maven plugin, or even the cli itself.

* Enabling tools like liquibase can be considered a "vendor locking", this is because we are creating sql scripts with vendor specific syntax. If this is a problem for you, consider enabling jpa automatic changes.
